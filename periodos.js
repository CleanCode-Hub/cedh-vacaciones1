...
superView = function () {
  ...
  const opts=data.areas.map(a=>`<option value="${a.id}">${esc(a.name)}</option>`).join(''); $('#new-user-area').innerHTML=`<option value="">Sin área</option>${opts}`; $('#areas-list').innerHTML=data.areas.map(a=>`<span class="area-chip">${esc(a.name)} · ${adminFor(a.id)?.name||'sin administrador'}</span>`).join('')||'<p class="empty">Aún no hay áreas.</p>'; const roles=['employee','admin','super']; $('#users-list').innerHTML=data.users.map(person=>`<tr><td>${esc(person.name)}</td><td>${esc(person.email)}</td><td><select data-role="${person.id}">${roles.map(item=>`<option value="${item}" ${item===person.role?'selected':''}>${role(item)}</option>`).join('')}</select></td><td><select data-area="${person.id}"><option value="">Sin área</option>${data.areas.map(a=>`<option value="${a.id}" ${a.id===person.areaId?'selected':''}>${esc(a.name)}</option>`).join('')}</select></td><td>${person.id!==currentUserId?`<button class="secondary danger" data-delete="${person.id}">Quitar</button>`:''}</td></tr>`).join('');
};
