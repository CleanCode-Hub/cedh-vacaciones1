...
superView = function () {
  ...
  const opts=data.areas.map(a=>`<option value="${a.id}">${esc(a.name)}</option>`).join(''); $('#new-user-area').innerHTML=`<option value="">Sin área</option>${opts}`; $('#areas-list').innerHTML=data.areas.map(a=>`<span class="area-chip">${esc(a.name)} · ${adminFor(a.id)?.name||'sin administrador'}</span>`).join('')||'<p class="empty">Aún no hay áreas.</p>'; $('#users-list').innerHTML=data.users.map(person=>`<tr><td>${esc(person.name)}</td><td>${esc(person.email)}</td><td>${role(person.role)}</td><td>${esc(area(person.areaId)?.name||'Sin área')}</td><td>${person.id!==currentUserId?`<button class="secondary danger" data-delete="${person.id}">Desactivar</button>`:''}</td></tr>`).join('');
};
